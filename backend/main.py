...
def matrix(
    ...
    swa = 9  # default swab area sqin — overwritten per PSE record if any exist
    rsa = 9  # default rinse sample area sqin — overwritten per PSE record if any exist
    ...
    for tgt in targets:
        ...
        total_area_in2 = sum(
            (e.surface_area_cm2 or 0) / 6.4516 for e in shared_equipment
        )
        ...
        if is_fixed_10ppm:
            ...
            sa = sum((e.surface_area_cm2 or 0) / 6.4516 for e in product_shared_objs)

            # Full 3-method MACO calculation for target products
            if src.pde_mg_day and tgt.max_daily_dose_mg and tgt.max_daily_dose_mg > 0 and tgt.min_yield_kg:
                maco_pde = (
                    src.pde_mg_day *
            ...
            if src.min_therapeutic_dose_mg and tgt.max_daily_dose_mg and tgt.max_daily_dose_mg > 0 and tgt.min_yield_kg:
                maco_dose = (
...
def list_reports(
    ...
    query = (db.query(CleaningValidationReport, ProtocolArchive.doc_number)
               .outerjoin(ProtocolArchive, CleaningValidationReport.archive_id == ProtocolArchive.archive_id))
    if facility_id:
