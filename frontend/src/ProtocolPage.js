...
export default function ProtocolPage({ goHome, currentUser, role }) {
  ...
  const [geoRunResults, setGeoRunResults] = useState([
    ...
    { run_number: 2, impurity_results: [] },
    { run_number: 3, impurity_results: [] },
  ]);
  ...
  const handleProtocolSelect = async (protocol) => {
    ...
    // Load genotoxic impurity limits for this product and init geo run results
    try {
    ...
    return (
      <div ref={printRef} style={{ background: "white", boxShadow: "0 4px 28px rgba(0,0,0,0.32)", minHeight: "1123px" }}>
                          ...
                          return (
                            <>
                              <span style={{ fontSize: "12px" }}>
                                Rinse: <strong style={{ color: "#1d4ed8" }}>{fmtG(imp.governing_rinse_limit)} ppm</strong>
                                {imp.rinse_status && (
                              ...
                              <span style={{ fontSize: "12px" }}>
                                Swab: <strong style={{ color: "#1d4ed8" }}>{fmtG(imp.governing_swab_limit)} ppm</strong>
                                {imp.swab_status && (
                                  <span style={{ marginLeft: "6px", padding: "1px 6px", borderRadius: "3px", fontSize: "10px",
        ...
        <div style={S.docFooter} className="doc-footer">
          <span>Falcon — Confidential | Cleaning Validation Protocol</span>
  ...
  const renderArchivedReport = () => {
    ...
    return (
      <div ref={reportPrintRef} style={{ background: "white", boxShadow: "0 4px 28px rgba(0,0,0,0.32)", minHeight: "1123px" }}>
        ...
        {/* Footer */}
        <div style={S.docFooter} className="doc-footer">
          <span>Falcon — Confidential | Cleaning Validation Report</span>
          <span>{product_name} — {facility_name}</span>
                              ...
                              <React.Fragment key={impIdx}>
                                ...
                                <tr style={{ background: impIdx % 2 === 0 ? "white" : "#fffbf0" }}>
                                  <td rowSpan={2} style={{ ...sampCell, fontWeight: "600", verticalAlign: "middle",
                                    color: "#92400e", borderRight: "2px solid #fcd34d" }}>
                                    {imp.impurity_name}
                                    <div style={{ fontWeight: "normal", color: "#b45309", fontSize: "10px" }}>
                                      PDE: {imp.pde_ug_day} μg/day
                                    </div>
                                  </td>
                                  <td style={sampCell}>Rinse</td>
                                  <td style={{ ...sampCell, textAlign: "center", fontWeight: "700", color: "#1d4ed8" }}>
                                    {fmtGeo(imp.governing_rinse_limit)}
                                  </td>
                                ...
                                <tr style={{ background: impIdx % 2 === 0 ? "#fafafa" : "#fffbf0" }}>
                                  <td style={sampCell}>Swab</td>
                                  <td style={{ ...sampCell, textAlign: "center", fontWeight: "700", color: "#1d4ed8" }}>
                                    {fmtGeo(imp.governing_swab_limit)}
                                  </td>
                                  {geoRunResults.map((run, runIdx) => {
                                    const runImp = run.impurity_results[impIdx];
                                    const st = compareResultToLimit(runImp?.swab_result_ppm, imp.governing_swab_limit);
